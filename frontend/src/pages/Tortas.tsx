...
import TortaModal from '@/components/tortas/TortaModal';
import { motion } from "framer-motion";
import { Search } from "lucide-react";
type Props = { showHeader?: boolean };

const Tortas: React.FC<Props> = ({ showHeader = true }) => {
  ...
  const [modalTorta, setModalTorta] = useState<number | null>(null);
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
          {filteredData.map((torta) => (
            <TortaCard
              ...
              onToggle={() => {
                // open modal when the torta has medidas
                if (torta.CantidadMedidas > 0) {
                  setModalTorta(torta.IdTorta);
                } else {
                  setTortaSeleccionada((prev) => (prev === torta.IdTorta ? null : torta.IdTorta));
                }
                setMedidasSeleccionadas((prev) => ({ ...prev, [torta.IdTorta]: null }));
              }}
              medidaSeleccionada={medidasSeleccionadas[torta.IdTorta] || null}
              onMedidaSelect={(medidaId) => {
                setMedidasSeleccionadas((prev) => ({ ...prev, [torta.IdTorta]: medidaId }));
              }}
            />
          ))}
        </div>
      ...
      {/* Modal: mostrar detalles y medidas al hacer click */}
      <TortaModal
        open={modalTorta !== null}
        torta={data.find(t => t.IdTorta === modalTorta) || null}
        onClose={() => setModalTorta(null)}
      />
    </div>
  );
};
