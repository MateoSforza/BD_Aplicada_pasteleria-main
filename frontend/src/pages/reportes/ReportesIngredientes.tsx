...
// Aquí interpretamos el semáforo como riesgo/costo unitario: más alto = peor (rojo)
const DEFAULT_HIGH = 80 // >= high => rojo
const DEFAULT_MED = 50 // >= med => amarillo

const ReportesIngredientes: React.FC = () => {
  ...
  const rows: Row[] = useMemo(() => {
    ...
    const arr = ingredientes.map((ing) => ({
      id: String(ing.idIngrediente),
      nombre: ing.nombre,
      unidad: ing.unidadCompra,
      precioUnitario: Number(ing.precioUnitario ?? 0),
      scorePct: 0,
    }))
  ...
  const filtered = useMemo(() => {
    ...
    if (!term) return rows
    return rows.filter(r => r.nombre.toLowerCase().includes(term) || r.unidad.toLowerCase().includes(term))
  }, [rows, q])
  ...
  const exportCsv = () => {
    const cols = ['Ingrediente','Unidad','PrecioUnitario','ScorePct']
    const lines = [cols.join(',')]
    filtered.forEach(r => {
      const line = [r.nombre, r.unidad, r.precioUnitario, r.scorePct].map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')
      lines.push(line)
    })
    ...
    a.href = url
    a.download = `reportes_ingredientes_${new Date().toISOString().slice(0,10)}.csv`
  ...
  if (isLoading) return <div className="text-primary-900">Cargando ingredientes...</div>
  if (isError) return <div className="text-red-600">Error al cargar los ingredientes.</div>
    ...
    <div className="text-primary-900">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <input
            type="text"
            placeholder="Buscar ingrediente o unidad..."
            value={q}
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="text-left text-sm text-primary-700">
              ...
              <th className="px-4 py-3">Score % (relativo)</th>
...
export default ReportesIngredientes
